...
async function fetchSerpAdvanced(keyword: string, locationCode: number = 2250): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`DataForSEO SERP Advanced error: ${res.status}`)
...
export async function analyzeIntent(keyword: string, locationCode: number = 2250): Promise<IntentAnalysis> {
  return getOrFetch<IntentAnalysis>(
    ...
    Infinity,
    async () => {
      log.info(`Analyzing intent for "${keyword}"`)
      ...
      log.info(`Intent classified for "${keyword}": ${classification.type} (confidence: ${classification.confidence})`)
...
async function fetchKeywordOverviewForLocation(keyword: string, locationCode: number): Promise<LocationMetrics> {
  ...
  if (!res.ok) throw new Error(`DataForSEO Keyword Overview error: ${res.status}`)
  const json = await res.json()
...
async function fetchGoogleAutocomplete(keyword: string): Promise<string[]> {
  ...
  if (!res.ok) throw new Error(`DataForSEO Autocomplete error: ${res.status}`)
  const json = await res.json()

  const items = json.tasks?.[0]?.result?.[0]?.items ?? []
  return items.map((i: any) => i.suggestion ?? i.keyword ?? '').filter(Boolean)
}
