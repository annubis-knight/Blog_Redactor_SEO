...
const ROLES: Record<string, { domain: Domain; role: string }> = {
  ...
  'humanize-section': { domain: 'Rédaction', role: 'Relecture d’une section : retire les tics d’écriture IA, corrige la langue (franglais, accords, typographie)' },
  'enrich-sources': { domain: 'Rédaction', role: 'Passe sources : remplace les « à sourcer » d’un chapitre par des données trouvées par la recherche web, liens réels' },
  'enrich-exemples': { domain: 'Rédaction', role: 'Passe exemples : ajoute un exemple en situation à un chapitre' },
