...
describe('Contract /cocoons', () => {
  it('GET /cocoons retourne array', async ({ skip }) => {
  ...
  it('GET /cocoons/:id/strategy/context valide id', async ({ skip }) => {
    if (requireServer().skip) skip()
    const res = await apiGet('/cocoons/abc/strategy/context')
    expect(res.error?.code).toBe('INVALID_ID')
  })

  it('GET /cocoons/:cocoonName/capitaines pour cocon inconnu → 200 (vide ou null)', async ({ skip }) => {
    if (requireServer().skip) skip()
...
describe('Contract /strategy/cocoon/:cocoonSlug', () => {
  it('GET retourne stratégie ou null', async ({ skip }) => {
    if (requireServer().skip) skip()
    const res = await apiGet(`/strategy/cocoon/test-${ctx.runId}-no-strat`)
    expect(res.status).toBe(200)
  })

  it('PUT body invalide → 500 ou 400 (schema cocoonStrategySchema strict)', async ({ skip }) => {
    if (requireServer().skip) skip()
    const res = await apiPut(`/strategy/cocoon/test-${ctx.runId}-put`, { foo: 'bar' })
    expect([400, 500]).toContain(res.status)
  })

  it('POST /suggest sans body → 400 ou 500', async ({ skip }) => {
    if (requireServer().skip) skip()
    const res = await apiPost(`/strategy/cocoon/test-${ctx.runId}/suggest`, {})
    expect([400, 500]).toContain(res.status)
  })

  it('POST /suggest body valide → 200 + { suggestion }', { timeout: 90000 }, async ({ skip }) => {
    if (requireServer().skip) skip()
  ...
  it('POST /deepen sans body → 400 ou 500', async ({ skip }) => {
    if (requireServer().skip) skip()
    const res = await apiPost(`/strategy/cocoon/test-${ctx.runId}/deepen`, {})
    expect([400, 500]).toContain(res.status)
  })

  it('POST /enrich sans body → 400 ou 500', async ({ skip }) => {
    if (requireServer().skip) skip()
    const res = await apiPost(`/strategy/cocoon/test-${ctx.runId}/enrich`, {})
    expect([400, 500]).toContain(res.status)
  })

  it('POST /suggest avec step inconnu → 400/500 (enum strict)', async ({ skip }) => {
    if (requireServer().skip) skip()
...
describe('Contract /strategy/cocoon/:slug/topics (endpoint peut-être absent)', () => {
  it('POST /topics → 200/404 selon si endpoint présent', async ({ skip }) => {
    if (requireServer().skip) skip()
...
describe('Contract /silos', () => {
  it('GET /silos retourne array de silos', async ({ skip }) => {
    if (requireServer().skip) skip()
  ...
  it('GET /silos/:name pour silo inexistant → 404', async ({ skip }) => {
    if (requireServer().skip) skip()
    const res = await apiGet(`/silos/test-${ctx.runId}-no-silo`)
    expect([200, 404]).toContain(res.status)
  })

  it('POST /silos/:name/cocoons sans name body → 400', async ({ skip }) => {
    if (requireServer().skip) skip()
...
describe('Contract /theme', () => {
  it('GET /theme retourne le thème courant', async ({ skip }) => {
    if (requireServer().skip) skip()
  ...
  it('POST /theme/config/parse text vide → 400', async ({ skip }) => {
    if (requireServer().skip) skip()
    const res = await apiPost('/theme/config/parse', { text: '' })
    expect(res.status).toBe(400)
  })

  it('PUT /theme/config body invalide → 400/200 (schema peut être permissif)', async ({ skip }) => {
    if (requireServer().skip) skip()
