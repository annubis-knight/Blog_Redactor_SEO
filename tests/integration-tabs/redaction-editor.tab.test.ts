...
describe('Tab redaction/editor — Content CRUD', () => {
  it('GET /articles/:id/content retourne 200 même pour article neuf', async ({ skip }) => {
  ...
  it('PUT /articles/:id sauvegarde content + GET le retrouve', async ({ skip }) => {
    if (requireServer().skip) skip()
    const silo = await ctx.getSilo()
  ...
  it('PUT /articles/:id/status met à jour status', async ({ skip }) => {
    if (requireServer().skip) skip()
    const silo = await ctx.getSilo()
...
describe('Tab redaction/editor — Génération via stream IA', () => {
  it('POST /generate/outline sans body → 400 VALIDATION_ERROR', async ({ skip }) => {
    if (requireServer().skip) skip()
