...
> ⚠️ **ARCHIVED 2026-05-13** — Les 23 drifts détectés pendant la refonte
> `docs/prd-split-spec-design` ont tous été soldés (10 dans le Groupe D,
> 7 dans le Groupe A, 1 dans le Groupe B via décision produit retirant
> les checks Cerveau+Rédaction, 5 dans le Groupe C via chantiers tech-debt
> dédiés). Ce document est conservé pour l'historique narratif et la
> traçabilité des décisions. **Pour les drifts détectés dans de futures
> migrations**, consigner dans `_bmad-output/planning-artifacts/drift-code-vs-doc.md`
> (template léger, vide à la base).

# Drift code vs doc — chantier `docs/prd-split-spec-design` (ARCHIVÉ)
