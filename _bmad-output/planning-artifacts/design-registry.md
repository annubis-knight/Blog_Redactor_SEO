...
- FAQ en fin d'article (`faqInsertIndex` = `MAX_SAFE_INTEGER`, moins de deux H2) : `anchor` vide, seul le refus d'une seconde FAQ protège l'insertion. Le message `stale` du panneau (« Le chapitre a changé… relancez la passe », [EnrichmentPanel.vue:164](../../src/components/panels/EnrichmentPanel.vue)) vaut aussi quand une FAQ existe déjà, où relancer ne propose rien.
...
- **Image par adresse** (commit `093ee57`) : pas d'envoi de fichier ni de stockage d'images dans l'outil ; la photo vit sur le site du client (`/…`) ou ailleurs sur le web (`https://…`). Remplacer garde le texte alternatif proposé par la passe Images, qui décrit ce que la photo doit montrer.

**Limites connues**
- Aucune insertion de tableau à la main.
- Le bouton « Image » ne permet pas de modifier le texte alternatif d'une image existante ; une adresse refusée ou un texte alternatif vide n'affiche aucun message (`EditorToolbar.vue:22,28`).
...
**Limites connues**
- Un lien retiré n'est signalé qu'au journal du serveur ([action.routes.ts:77](../../server/routes/generate/action.routes.ts)), pas à l'utilisateur.
- `usage.stopReason` n'est pas lu : une action coupée (plafond de 2 048 jetons, `pause_turn` d'une recherche) est rendue comme complète, contrairement aux passes (`enrich-truncated`).
- `req.socket.setTimeout(0)` n'est pas posé, contrairement aux autres routes longues (`enrich.routes.ts:33`, `humanize-section.routes.ts:67`) ; aucun délai n'étant configuré côté serveur, rien ne coupe la connexion aujourd'hui.
