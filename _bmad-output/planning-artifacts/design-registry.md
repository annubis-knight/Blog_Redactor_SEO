...
- Écran (commit `fb92b46`) :
  ...
  - **Menu « Générer avec Claude » (U7, branche `feat/cerveau-generer-au-choix`)** :
    - [src/components/production/brain/GenerateCocoonMenu.vue](../../src/components/production/brain/GenerateCocoonMenu.vue) — bouton `brain-generate-menu` (« Générer avec Claude ▾ », « Génération... » et désactivé pendant la carte) et menu `brain-generate-options` (`role="menu"`). Choix `brain-generate-pillar` : émet `pillar`, désactivé sans `canStartPillar`, avec l'explication `hasPillar` (« Le pilier existe déjà… ») ou « arbre pas prêt ». Choix `brain-generate-articles` : émet `map`. Échap (sur tout le menu) et un clic à côté (`mousedown` sur le document, écouté seulement menu ouvert) referment ; à l'ouverture, le premier choix possible prend le focus.
    - `BrainArticleProposalView.vue` : titre « Carte indicative du cocon », props `canStartPillar` / `hasPillar`, événement `start-pillar`.
    - `CocoonTreeBuilder.vue` : `canStartPillar = !isLoadingTree && !treeError && !hasPillar`, même expression pour son bouton « Créer le pilier » et pour le menu. `startPillar()` fait défiler jusqu'au constructeur, puis appelle `proposePillar()` sauf pendant une proposition ou une création. `defineExpose({ startPillar, canStartPillar, hasPillar })`.
    - `BrainPhase.vue` : référence `treeBuilder` au constructeur ; la carte reçoit `treeBuilder?.canStartPillar` et `treeBuilder?.hasPillar`, et `@start-pillar` appelle `treeBuilder?.startPillar()`.
    - Tests : `generate-cocoon-menu.test.ts`, `cocoon-tree-builder.test.ts` (startPillar), `brain-phase-architecture.test.ts` (câblage), `production-phases.test.ts`, `cerveau.parcours.test.ts` (le pilier naît par le menu ; menu grisé une fois le pilier créé).
