...
Les deux sections récap (toggle) en tête du Moteur — **"Articles suggérés"** et **"Articles publiés"** — segmentent la liste du cocon par phase workflow et ne doivent **jamais** se chevaucher :

- **Articles suggérés** = articles encore au stade brainstorm Cerveau, alimentés exclusivement par `strategyStore.strategy.proposedArticles` (synthétisés avec `phase: 'proposed'`).
- **Articles publiés** = articles ayant quitté l'état proposé pour entrer dans le pipeline éditorial. Critère **strict** : `phase IN ('redaction', 'published')`. Les articles `phase = 'moteur'` (en cours de travail Moteur, déjà promus mais pas encore au stade rédaction) restent dans la liste principale de sélection — pas dans "publiés".

**Implémentation** : la dérivation est faite **côté backend** dans `loadArticlesDb` (`server/services/infra/data.service.ts`) qui expose, en plus de `cocoon.articles` (liste complète), un champ dérivé `cocoon.publishedArticles: Article[]` filtré sur les phases ci-dessus. Le frontend (`MoteurView.vue`) lit ce champ directement — pas de filtre Vue. Motivation : éviter qu'un autre consommateur frontend (LinkingMatrix, BriefStructureStep, useArticleProposals) duplique ce filtre par méprise, et garantir un contrat API explicite.

**Pourquoi pas une seconde requête SQL** : `publishedArticles ⊂ articles`, dérivation en O(n) sur un set déjà hydraté. Ajouter un WHERE sur la requête principale casserait les 3 consommateurs qui ont besoin de la liste complète.

**Critères d'acceptation testables** :
- AC.RECAP-PUB.1 (contract-api) : `loadArticlesDb()` sur un cocon avec 3 articles `phase='proposed'` + 2 `phase='moteur'` + 2 `phase='redaction'` + 1 `phase='published'` retourne `cocoon.articles.length === 8` et `cocoon.publishedArticles.length === 3`.
- AC.RECAP-PUB.2 (contract-api) : `cocoon.publishedArticles` ne contient aucun article avec `phase === 'proposed'` ni `phase === 'moteur'`.
- AC.RECAP-PUB.3 (type) : `Cocoon.publishedArticles: Article[]` typé non-optionnel (toujours présent dans la payload, même vide).
- AC.RECAP-PUB.4 (non-régression UI) : pour un cocon contenant uniquement des articles `phase='proposed'`, la section "Articles publiés" du `MoteurContextRecap` est vide (aucun groupe Pilier/Intermédiaire/Spécialisé rendu).

**Source bug initial** : 2026-05-11 — l'utilisateur génère 13 articles via Cerveau (insertion BD avec `phase='proposed'` par défaut, cf. `data.service.ts:312`), et ces 13 articles apparaissent à la fois dans "Articles suggérés" (via `proposedArticles` du strategyStore) **et** dans "Articles publiés" (via `cocoon.articles` non filtré). Cette FR formalise le contrat anti-doublon.

**Voir aussi** : FR-MOT-CONTEXT-RECAP (composant `MoteurContextRecap.vue` qui consomme cette segmentation), FR-MOT-PHASES (référentiel des valeurs `phase`).
...
| `MoteurContextRecap.vue` | Recap contexte stratégie (cocoon + capitaine) + sections récap "Articles suggérés" / "Articles publiés" | tous onglets Moteur | FR-MOT-CONTEXT-RECAP, FR-MOT-RECAP-PUBLISHED |
