...
| Suites | `2ca3d32` fix(redaction): suites relevées à la clôture de l'enrichissement | Action coupée → `error` `ACTION_TRUNCATED` (`action.routes.ts`), `streamedResult` vidé sur erreur (`useContextualActions.ts`), « Accepter » grisé sans résultat (`ActionResult.vue`) ; `removedLinks` dans `done` + `actionNotice` ; `req.socket.setTimeout(0)` si recherche web ; `EditorToolbar.setImage` : `imageNotice`, texte alternatif relu au remplacement ; `EnrichmentPanel` : `enrich-images-to-provide`, `EnrichmentItem.staleReason` ; `targetWordCount` dans le `done` du premier jet → `editorStore.lastDraftTargetWordCount` → `briefStore.setRetainedWordCount` ; `describeUnknownTypeFaq` (`article-type-rules.ts`) ; en-tête de `enrichment.ts` ; `WEB_SEARCH_TOOL` supprimé. Tests : `generate.routes.test.ts`, `useContextualActions.test.ts`, `ActionResult.test.ts`, `EditorToolbar.test.ts`, `enrichment-panel.test.ts`, `useArticleGeneration.test.ts`, `enrichment.service.test.ts` |
...
- ~~**Actions contextuelles**~~ (R21, soldé) : `sources-chiffrees` et `exemples-reels` cherchaient sans ville et personne ne vérifiait leurs liens. Désormais `webSearchTool(zone)`, texte accumulé, liens filtrés par `keepKnownLinks`, un seul `chunk`, keep-alive 15 s (`action.routes.ts:51-80`). ~~Reste : un lien retiré n'est que journalisé.~~ Signalé à l'écran depuis les suites de clôture de C5b, commit `2ca3d32` (`removedLinks`).
