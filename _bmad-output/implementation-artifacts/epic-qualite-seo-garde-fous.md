...
**Statut** : livrée par C4 (versée au PRD et au registre, 2026-09-25). Texte amendé à la livraison, le PRD fait foi : aucune règle de nombre de FAQ n'existe encore (la passe FAQ, livrée par C5b, demande 3 à 6 questions dans son prompt, quel que soit le type : checklist R22) ; un seuil d'alerte (« contenu mince ») reste distinct de la borne basse de la cible, dans la même source ; le vérificateur de publication suit le plafond du type, pas la longueur visée.
...
**Statut** : livrée par C5b (versée au PRD et au registre, 2026-09-25). Texte amendé à la livraison, le PRD fait foi : panneau « Enrichir » dans les deux vues ; une passe = un appel par chapitre visé (corps de l'article pour Exemples, Tableaux, Images ; une FAQ unique insérée avant la conclusion) ; un seul vérificateur pour toutes les passes (`verifyEnrichment`, ⛔ / 🔴 / 🟠), une proposition ⛔ ne s'accepte pas ; accepter remplace un chapitre et enregistre aussitôt, jamais un chapitre modifié depuis ; une image proposée est une place « à fournir », refusée à la publication. Limites : H1 non protégé (R17), contexte coupé et sans stratégie (R18), rien ne permet de fournir la photo (R20), nombre de questions de FAQ hors des règles par type (R22), FAQ insérée sans contrôle de fraîcheur (R23).
...
- [x] R16 — *(découvert en documentant C5a, 2026-09-25 ; **soldé côté serveur dans C5a** (commit `43ed323`) : la route du premier jet prend d'abord la cible choisie par l'utilisateur (micro-contexte), sinon celle de l'écran, sinon celle du type, et **enregistre la cible retenue** dans `article_micro_contexts.target_word_count` quand aucune n'était choisie (`retainTargetWordCount`, jamais d'écrasement) : la porte juge donc contre la valeur qui a guidé la rédaction. **Soldé côté écran dans C5b** (commit `57fe1e8`) : `briefStore.targetWordCount` = longueur choisie (lue par `GET /articles/:id/micro-context`, suivie par `setRetainedWordCount` depuis `BriefStructureStep`), sinon recommandation ; lue par `useArticleGeneration.wordCountTarget` (barre, écart, réduction, cible envoyée au premier jet), `SeoPanel` et `useSeoScoring` des deux vues. Tests : `brief.store.test.ts` (« longueur visée »), `useArticleGeneration.test.ts`. Cas limite restant : R24)* **Le premier jet et sa porte ne visaient pas la même longueur.** L'écran envoie `briefData.contentLengthRecommendation` (`useArticleGeneration.ts:69,114`, recommandation calculée à l'ouverture du brief et jamais enregistrée), que la route prend en priorité (`article-draft.routes.ts:118`) et que la barre de mots affiche ; la porte juge contre `article_micro_contexts.target_word_count`, sinon la règle du type (`gate.service.ts:223`). Quand elles diffèrent, un premier jet conforme à sa consigne reçoit `draft-length-off-target` et des `draft-section-off-budget`. Violation de la règle « même expression pour l'affichage et le calcul » (`.claude/CLAUDE.md` §2.0). À trancher : enregistrer la cible utilisée avec le premier jet, ou la faire lire à la porte.
- [ ] R17 — *(découvert en documentant C5b, 2026-09-25, non corrigé)* **Le H1 n'est pas protégé par les passes.** `verifyEnrichment` ne compare que les H2 et H3 (H2 seul pour une réécriture ; `shared/verifiers/enrichment.ts:126-127`) ; or le chapeau (chapitre -1, `shared/chapters.ts:19`) contient le H1, et la passe Sources comme la réécriture peuvent le viser. Un H1 réécrit (capitaine perdu) passe sans alerte ; la publication le rattraperait en 🔴 (`seo-capitaine-not-in-title`), pas la proposition.
- [ ] R18 — *(découvert en documentant C5b, non corrigé)* **Les passes voient un article tronqué et sans stratégie.** `articlePlainText(html, maxChars = 12000)` (`shared/chapters.ts:51`) coupe le contexte vers 2 000 mots : la fin d'un pilier (conclusion comprise) n'est pas vue par les passes ni par la réécriture, qui doit pourtant « ne pas répéter un autre chapitre ». Et `enrichment.service.ts:64,90` appelle `loadPrompt` sans `cocoonSlug` : `{{strategy_context}}` est vide, ni cible, ni douleur, ni angle.
- [ ] R19 — *(découvert en documentant C5b, à vérifier en réel)* `toStopReason` (`claude.service.ts:163-167`) range `pause_turn` — arrêt d'un tour qui utilise un outil serveur comme la recherche web — dans `other` ; `enrich-truncated` ne se déclenche que sur `max_tokens`. Une passe Sources interrompue ainsi serait montrée comme complète.
- [ ] R20 — *(découvert en documentant C5b, non corrigé)* **Une image « à fournir » ne peut pas être fournie.** L'éditeur n'expose aucune commande d'insertion ou de remplacement d'image ni de tableau (`setImage`, `insertTable` absents de `src/`) ; la porte de publication dit « remplacez l'image ou retirez-la » (`shared/verifiers/publish.ts:157`) mais seule la suppression est possible dans l'outil.
- [ ] R21 — *(reste de R6, C5b)* Les actions `sources-chiffrees` / `exemples-reels` utilisent `WEB_SEARCH_TOOL` sans ville (`action.routes.ts:51`) ; leurs `usage.webSources` partent dans l'événement `done` mais ni `useContextualActions` ni `ActionResult` ne les lisent : aucun lien d'une action n'est comparé aux résultats de la recherche.
- [ ] R22 — *(découvert en documentant C5b)* Le nombre de questions de FAQ (3 à 6) est écrit dans `server/prompts/enrich-faq.md:16`, pour tous les types, au lieu d'une règle de `shared/constants/article-type-rules.ts` (FR-INFRA-TYPE-RULES-SSOT) ; `verifyEnrichment` n'en contrôle pas le nombre.
- [ ] R23 — *(découvert en documentant C5b)* Accepter une FAQ ne vérifie pas que l'article a changé depuis la proposition (`enrichment.store.ts:159-161`) : elle s'insère à l'index calculé à la proposition. Les autres passes, elles, passent en « chapitre modifié depuis ».
- [ ] R24 — *(cas limite de R16, C5b)* Sans longueur choisie, la cible retenue par la route du premier jet n'est relue par l'écran qu'au `fetchBrief` suivant (`brief.store.ts:121-133`) ; si la recommandation de l'IA arrive après le lancement du premier jet, la barre affiche cette nouvelle valeur alors que la porte juge la valeur retenue, jusqu'au rechargement.
