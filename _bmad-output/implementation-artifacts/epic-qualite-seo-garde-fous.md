...
**Statut** : livrée par C5b (versée au PRD et au registre, 2026-09-25). Texte amendé à la livraison, le PRD fait foi : panneau « Enrichir » dans les deux vues ; une passe = un appel par chapitre visé (corps de l'article pour Exemples, Tableaux, Images ; une FAQ unique insérée avant la conclusion) ; un seul vérificateur pour toutes les passes (`verifyEnrichment`, ⛔ / 🔴 / 🟠), une proposition ⛔ ne s'accepte pas ; accepter remplace un chapitre et enregistre aussitôt, jamais un chapitre modifié depuis ; une image proposée est une place « à fournir », refusée à la publication. Les limites relevées à la livraison (R17, R18, R20, R22, R23) sont soldées par le commit `093ee57` : H1 protégé, article entier (30 000 caractères) et stratégie de l'article transmis, bouton « Image » pour fournir la photo, nombre de questions de FAQ fixé par le type, FAQ non insérée si la conclusion a changé.
...
**Statut** : réservée — **non livrée par C5b** (2026-09-25) : aucun code de maillage n'a changé ; le panneau « Maillage » reste celui de FR-RED-INTERNAL-LINKING. Chantier à choisir (C7, où naissent les enfants du pilier, semble le plus naturel).
...
| FR-RED-INTERNAL-LINKING | superseded (pas encore : FR-RED-LINKING-MANUAL n'est pas livrée par C5b) | FR-RED-LINKING-MANUAL | à choisir |
...
- [ ] M18 — *(découvert en documentant C6, 2026-09-25, non corrigé)* L'onglet Structure relit la SERP du capitaine dès son montage et à chaque changement d'article (`StructureHnPanel.vue:104-108,115-118` → `useStructureHn.ts:98-118`, `POST /serp/analyze`) : au-delà de 7 jours de cache, c'est une nouvelle analyse payante (DataForSEO + lecture de 10 pages) déclenchée sans clic, et `computeSmartTab` ouvre directement cet onglet quand les lieutenants sont validés. Contraire à FR-MOT-NO-AUTO-ACTION. À trancher : lire la récurrence seulement au clic « Générer », ou lire la SERP en cache sans jamais la rafraîchir.
- [ ] M19 — *(découvert en documentant C6, non corrigé)* `moteur:hn_locked` n'est pas revérifié quand le capitaine ou les lieutenants retenus changent après la validation (aucun watcher dans `StructureHnPanel.vue`, contrairement à `LieutenantsPanel.vue:414-418` et au Lexique) : l'étape reste verte alors que la porte signalerait un lieutenant absent ou un H1 sans le nouveau capitaine. Seule la publication rejoue la porte.
- [ ] M20 — *(découvert en documentant C6, non corrigé)* `useStructureHn.prepareValidation` : la recommandation de longueur part sans être attendue (`void recommendWordCount(id)`, ligne 203) alors que le commentaire (199) dit que « le sommaire et la longueur partent AVANT l'étape » ; un échec du `PUT /articles/:id { outline }` n'est que journalisé et l'étape est quand même demandée (200-202) ; valider remplace un sommaire retouché dans la Rédaction.
- [ ] M21 — *(découvert en documentant C6, non corrigé)* `saveStructure` écrit la structure dans le store avant la réponse du serveur, sans retour arrière si elle échoue (`article-keywords.store.ts:250`) : après un refus, `dirty` est faux et l'écran croit la structure enregistrée.
...
- [ ] C5 · R13 — La brief récupère les questions PAA du mot-clé **pilier du cocon** (`brief.store.ts:77`), pas du capitaine de l'article : un intermédiaire est rédigé avec les questions du pilier. *(Non traitée par C5b ; toujours vrai : `pilierKeyword` dans `brief.store.ts`.)*
...
- [ ] P6 — *(découvert en documentant C6, 2026-09-25, à trancher)* La publication rejoue la porte `hn-lock` sur `article_keywords.hn_structure` (`gate.service.ts:287`) : un article sans structure enregistrée — rédigé sans passer par l'onglet Structure, ou d'avant C6 sans structure — reçoit `hn-lock:hn-empty` ⛔ et ne peut plus être publié, même avec une raison. C3 avait passé `lexique-empty` en 🔴 pour ce cas. Décision produit : exiger une structure (et le dire à l'écran de publication), ou passer `hn-empty` en 🔴 à la publication.
...
- [ ] T13 — *(découvert en documentant C6, 2026-09-25)* Le cliquet `itSkip` de `tests/unit/coherence/test-quality.test.ts` (ligne 149) est resté à 42 alors que C6 a retiré deux `it.skip` (40 au commit `94c7e91`) : à abaisser à 40 pour que les deux ne puissent pas revenir en silence.
