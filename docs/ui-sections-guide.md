...
>   - **Fix critique Sprint 0.1 — Hydratation carousel Capitaine**. `getArticleKeywords` construit désormais `richCaptain.exploredKeywords` dès qu'au moins une ligne `captain_explorations` existe, même si `article_keywords.capitaine` est vide. Avant : les 29 explorations en DB n'apparaissaient jamais dans le carousel après refresh tant que le capitaine n'était pas verrouillé.
...
- **D5 — TabCachePanel DB-first** : les badges « contenu disponible » sont basés sur `GET /articles/:id/explorations/counts` (UNION ALL de 7 compteurs : `radar`, `captain`, `lieutenants`, `paa`, `lexique` lus dans leurs tables `*_explorations`, `local` et `contentGap` lus dans `keyword_metrics` ; plus de compteur `intent` depuis le 2026-09-25, épopée qualité SEO M3), pas sur des flags `locked`.
...
| **TabCachePanel (D5)** | `TabCachePanel` | Chips résumés des onglets ayant des données en cache ou des explorations DB. Depuis Sprint 12, les compteurs viennent de `GET /articles/:id/explorations/counts` (UNION ALL de 7 compteurs depuis le 2026-09-25 — M3 a retiré `intent` ; le panel n'en affiche que 4). Bouton compagnon « Vider cache externe » → `DELETE /articles/:id/external-cache`. **Maj 2026-05-01** : Discovery retiré du panel (modèle de persistance cross-article seed-based, incompatible avec un pilotage par articleId). Le panel affiche désormais 4 chips : Radar, Capitaine, Lieutenants, Lexique. | Au moins un onglet cached OU `count > 0` par type | Nav onglet au clic, purge cache |
...
| **Root keywords (F4 — suppression arbitraire)** | `CaptainInteractiveWords` + `KeywordWords` | Chaque mot du capitaine est cliquable individuellement pour le désactiver/réactiver. Plus de troncature droite uniquement. **Sanctuarisation (mai 2026)** : les 2 premiers mots significatifs (non-stopwords) sont visuellement ancrés (`kw-word--sanctuary`, font-weight 600, pas de soulignement, curseur normal) — clics court-circuités. Les stopwords ne consomment pas les slots. Capitaines ≥ 3 mots → toujours interactifs (cohérence visuelle, ne dépend plus du cache des racines). Garde-fou résiduel : minimum 2 mots significatifs non-stopwords après désactivation (`kw-word--locked`). | Clic sur un mot → emit `word-toggle` avec `indices: number[]` | Met à jour `activeWordIndices` de l'entry carousel, switch vers la variante correspondante si elle existe en `rootVariants` |
...
- **Onglet SERP Data** (`SerpDataTab`) : overview Volume/KD/CPC/Compétition + collapsibles Top 10 + PAA + mots-clés associés ; refresh manuel → `POST /dataforseo/brief` (forceRefresh). Depuis le 2026-09-25 (épopée qualité SEO, R13), ces données portent sur le mot-clé **de l'article** (capitaine verrouillé, sinon mot-clé suggéré ; `briefStore.serpKeyword`), plus sur le mot-clé pilier du cocon ; sans mot-clé, l'onglet n'a pas de données.
...
| `/articles/:id/explorations/counts` | GET | **Sprint 12 (D5)** — 7 compteurs en UNION ALL (`radar`, `captain`, `lieutenants`, `paa`, `lexique`, `local`, `contentGap` ; plus de `intent` depuis le 2026-09-25, M3). Utilisé par TabCachePanel. |
...
| `/dataforseo/brief` | POST | Données DFS brief (+ forceRefresh), sur le mot-clé de l'article (R13, 2026-09-25) |
...
- **Pattern `*_explorations` (Sprints 9-13)** : chaque nouvelle donnée article-specific doit suivre le pattern existant :
  ...
  5. Comptage via `GET /articles/:id/explorations/counts` (ajouter le UNION ALL ; 7 compteurs au 2026-09-25). À l'inverse, une table qui perd son producteur perd aussi sa lecture et son compteur (cas de `keyword_intent_analyses`, épopée qualité SEO M3).
...
sequenceDiagram
    ...
    participant CV as CaptainPanel.vue
...
| [CaptainInteractiveWords.vue](../src/components/moteur/CaptainInteractiveWords.vue) | 12 | [captain-interactive-words.test.ts](../tests/unit/components/captain-interactive-words.test.ts) | wrapper interactif ; isLocked dérivé ; emit lock/unlock/word-toggle ; **REGRESSION GUARD** displayMode hardcodé "relevance" ; **REGRESSION GUARD** keyword ≥ 3 mots → interactiveWords toujours présent + `lockedLeftWords: 2` (sanctuarisation des 2 premiers significatifs) |
...
| [CaptainPanel.vue](../src/components/moteur/CaptainPanel.vue) | 75 | [captain-validation.test.ts](../tests/unit/components/captain-validation.test.ts) | composant racine onglet Capitaine ; validation + carousel + lock workflow complet |
| [CaptainInput.vue](../src/components/moteur/CaptainInput.vue), [CaptainVerdictPanel.vue](../src/components/moteur/CaptainVerdictPanel.vue), [CaptainLockPanel.vue](../src/components/moteur/CaptainLockPanel.vue) | 13 | [captain-sub-components.test.ts](../tests/unit/components/captain-sub-components.test.ts) | composants atomiques Capitaine ; bouton Lock toujours cliquable (un NO-GO passe par l'alarme 🔴 de la porte capitaine, FR-CAP-LOCK-GATE) |
| [KeywordWords.vue](../src/components/intent/KeywordWords.vue) | 19 | [keyword-words.test.ts](../tests/unit/components/keyword-words.test.ts) | mots cliquables du keyword ; toggle actif/inactif ; **`lockedLeftWords` sanctuarisation** : N premiers mots significatifs visuellement non-cliquables (ancrage racine capitaine) ; stopwords ignorés des slots ; tooltip explicite |

### 12.2 Composants Moteur — Lieutenants

| Composant | Tests | Fichier | Couverture |
|---|---|---|---|
| [LieutenantsPanel.vue](../src/components/moteur/LieutenantsPanel.vue) | 111 | [lieutenants-selection.test.ts](../tests/unit/components/lieutenants-selection.test.ts) (94) + [lieutenants-selection.gaps.test.ts](../tests/unit/components/lieutenants-selection.gaps.test.ts) (17) | composant racine onglet Lieutenants ; sélection cards ; lock workflow ; gates |
...
| Composant | Tests | Fichier | Couverture |
|---|---|---|---|
| [DiscoveryPanel.vue](../src/components/moteur/DiscoveryPanel.vue) | 44 | [keyword-discovery-tab.test.ts](../tests/unit/components/keyword-discovery-tab.test.ts) | onglet 1 du Moteur ; discovery + filtres + cache + classification IA |
| [RadarPanel.vue](../src/components/intent/RadarPanel.vue) | 35 | [douleur-intent-scanner.test.ts](../tests/unit/components/douleur-intent-scanner.test.ts) | onglet Radar ; scan + thermomètre + verdict |
| [LexiquePanel.vue](../src/components/moteur/LexiquePanel.vue) | 91 | [lexique-extraction.test.ts](../tests/unit/components/lexique-extraction.test.ts) (64) + [lexique-extraction.gaps.test.ts](../tests/unit/components/lexique-extraction.gaps.test.ts) (18) + [lexique-sort-by-alignment.test.ts](../tests/unit/components/lexique-sort-by-alignment.test.ts) (9) | TF-IDF par sections ; toggle tri par alignement douleur ; IA recommendations ; gates |
| [FinalisationPanel.vue](../src/components/moteur/FinalisationPanel.vue) | 11 | [finalisation-recap.test.ts](../tests/unit/components/finalisation-recap.test.ts) | gate vers Rédaction ; capitaine + lieutenants + lexique ; emit navigate-redaction |
...
| `TabCachePanel` (D5, Sprint 12) | `GET /articles/:id/explorations/counts` — UNION ALL de 7 compteurs (plus de `intent` depuis M3, 2026-09-25) | R |
