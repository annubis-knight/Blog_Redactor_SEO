...
- **Service SERP** `analyzeSerpCompetitors()` ([server/services/external/serp-analysis.service.ts](../../server/services/external/serp-analysis.service.ts)) — persiste désormais (Story C4) **uniquement** dans les 4 tables filles via `keyword-serp.service.ts` (transaction atomique : `upsertSerpResults` + `upsertSerpScrapes` + `upsertPaaQuestions`). Zéro re-requête si frais — freshness check sur `keyword_serp_results.fetched_at` (Story C2). Invariant **NFR-INT-SERP-ONCE** préservé.
...
| SERP stale (> 7j) + re-test article A, puis article B même mot-clé | `/serp/analyze` lit `keyword_metrics.serp_raw_json` miss → `analyzeSerpCompetitors()` → `upsertKeywordSerp()` | Atomique via upsert | Faible si une seule requête par mot-clé lors du freshness miss. |
