...
| **PainPoint saisie Cerveau (form article étape 2)** | — | `articles.pain_point = user_input` (UPDATE) | **Risque** : l'article passe de `painPoint = null` à `painPoint = "..."`. Tous les prompts Moteur doivent relancer après ce changement pour recalculer `relevanceScore` (cf. cas refresh ci-dessous). |
