...
- **Endpoint** `POST /api/serp/analyze` ([server/routes/serp-analysis.routes.ts:19-50](../../server/routes/serp-analysis.routes.ts)) — reçoit `{ keyword, articleLevel }`, consulte d'abord l'analyse en base (cache cross-article, TTL 7j), sinon scrape DataForSEO Top 10 concurrents. *(Précisé le 2026-09-25, C7, commit `1062072` : la relecture en base est `getSerpResultsFresh` (`keyword_serp_results`) **puis** `reconstructSerpAnalysisResult`, qui rend `null` si aucune page n'a été lue (`keyword_serp_scrapes` vide) — un relevé des premiers résultats, comme celui qu'écrivait la mesure des candidats du Cerveau, ne compte plus comme une analyse. La mesure des candidats écrit désormais dans `external_api_cache` (`serp-top`).)* Extrait headings H1/H2/H3 via regex, calcule récurrence + pourcentages, retourne `SerpAnalysisResult { competitors[], paaQuestions[], maxScraped, fromCache }`.
