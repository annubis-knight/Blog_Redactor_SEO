...
synced_with:
  ...
  - server/services/strategy/prompt-context.service.ts (date, zone, repères)
...
| **2. Contexte** | Ce que l'IA doit savoir : date, zone, stratégie, article, mots-clés | Variables globales du chargeur (`{{today}}`, `{{year}}`, `{{zone}}`, `{{zone_landmarks}}`, `{{strategy_context}}`) ; blocs fournis par la route (`{{strategyContext}}`, `{{keywordContext}}`, `{{microContext}}`, `{{themeContext}}`…) |
...
À venir : l'état du cocon (`{{cocoon_context}}`, chantier C7).
...
| `zone_landmarks` | Autres noms de la zone, quartiers et communes, lieux connus | table `local_entities` (les entreprises n'y sont pas reprises) |
